...
export function JobChecklistPhotosPanel({ jobId, metadata, canWrite = false }: { jobId: string; metadata: Record<string, unknown> | null; canWrite?: boolean }) {
  ...
  async function persist(nextChecklist: ChecklistItem[], nextPhotos: PhotoItem[]) {
    ...
    const res = await fetch(`/api/v1/services/jobs/${jobId}`, {
      ...
      body: JSON.stringify({ metadata: { ...(metadata ?? {}), checklist: nextChecklist, photos: nextPhotos } }),
    });
  ...
  async function addPhoto(e: React.FormEvent) {
    ...
    if (!url) return;
    const next = [...photos, { id: `p-${Date.now()}`, url, caption: photoCaption.trim() || undefined }];
    setPhotos(next);
    setPhotoUrl("");
    setPhotoCaption("");
