...
export function InsightsDashboard({ organisationName, intelligence }: { organisationName: string; intelligence: GeneratedIntelligence | null }) {
  return (
    <div className="space-y-6">
      {!intelligence ? (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-100/90">
          <p>Connect CRM, website, and finance systems so DigitalGate can notice patterns in your business activity.</p>
          <ResolutionAction href="/dashboard/settings/connected-services" mode="guided" label="Connect data" className="mt-3" showHint />
        </div>
      ) : null}
      ...
      {intelligence?.insights.length ? (
        <section className="dg-card">
          ...
          <p className="mt-1 text-sm text-slate-500">Interpreted observations from connected systems — not raw counts. See Analytics for the underlying numbers.</p>
          <ul className="mt-4 space-y-3">
            {intelligence.insights.map((insight, index) => (
              <li key={`${insight.text}-${index}`} className={`rounded-xl border px-4 py-3 text-sm ${toneClass(insight.tone ?? "neutral")}`}>{insight.text}</li>
            ))}
          </ul>
        </section>
      ) : null}
      ...
      {intelligence?.recommendedActions.length ? (
        <section className="dg-card">
          <h2 className="font-semibold text-white">Worth improving</h2>
          <p className="mt-1 text-sm text-slate-500">Every recommendation includes the shortest available path to resolve it.</p>
          <ul className="mt-4 space-y-3">
            {intelligence.recommendedActions.map((action) => (
              <li key={action.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-white">{action.label}</p>
                  <p className="text-sm text-slate-400">{action.impact}</p>
                </div>
                {action.href ? <ResolutionAction href={action.href} mode="guided" label="Fix now" /> : <ResolutionAction href="/dashboard/advisor" mode="guided" label="Get help fixing it" />}
              </li>
            ))}
          </ul>
        </section>
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-950/30 px-4 py-4 text-sm text-slate-400">
        Analytics shows the evidence. Insights interprets what DigitalGate notices. AI Advisor explains what to do about it.
        <div className="mt-3 flex flex-wrap gap-4">
          <Link href="/apps/analytics" className="text-sky-400 hover:underline">Open Analytics →</Link>
          <Link href="/dashboard/advisor" className="text-sky-400 hover:underline">Open AI Advisor →</Link>
        </div>
      </section>
