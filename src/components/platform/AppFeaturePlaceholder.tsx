...
type AppFeaturePlaceholderProps =
  ...
  | ({ href: string; itemId?: never; hideRoadmap?: boolean });
...
export function AppFeaturePlaceholder(props: AppFeaturePlaceholderProps) {
  ...
  const hideRoadmap = props.hideRoadmap === true;
  ...
  const moreItems = (related.length > 1 ? related : areaItems).filter((r) => r.id !== item.id);
    ...
    <>
      ...
      <header className="dg-page-header">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-3">
          ...
          {hideRoadmap ? null : <RoadmapStatusBadge status={item.status} />}
          {setupGuide ? (
            <Link
      ...
      <main className="flex-1 space-y-6 p-8">
        <div className="dg-card max-w-2xl border-dashed border-slate-700">
          ...
          <h2 className="mt-2 text-lg font-semibold text-white">
            {hideRoadmap ? "Not available yet" : "Coming to Platform Gen 2"}
          </h2>
          <p className="mt-2 text-sm text-slate-400">
            {hideRoadmap ? (
              <>
                This screen is in the product map so navigation stays complete.{" "}
                {item.description}
              </>
            ) : (
              <>
        ...
        {!hideRoadmap && moreItems.length > 0 ? (
