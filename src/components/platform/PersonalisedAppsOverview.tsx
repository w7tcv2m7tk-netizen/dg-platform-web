...
import { listTemplates } from "@dg/platform-core/industry";
...
const industryTemplateHrefById = new Map(listTemplates().map((template) => [template.id, template.primaryHref]));
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-400">
              DigitalGate only shows the parent Industry Apps and sub-industry Apps relevant to this organisation. Other industries stay out of the way until you choose to add them.
            </p>
          </div>
      ...
      <section>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-slate-400">Only active parent industries and selected sub-industry Apps are shown here.</p>
        </div>
        ...
        {industryPlatforms.length ? (
                ...
                {specialisations.length ? (
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {specialisations.map((spec) => {
                      // Route by exact sub-industry identity first. A shared runtime appId
                      // (services, finance, creator, automotive, etc.) must never collapse
                      // distinct child Apps onto the same generic entry point.
                      const href = industryTemplateHrefById.get(spec.id) ?? appHref(spec.appId);
                      return (
                  ...
                  <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/50 p-4">
                    <p className="text-sm text-slate-300">This Industry App is active. Choose the sub-industry App you operate so DigitalGate can tailor the workspace instead of showing every available sibling.</p>
                    <Link href="/dashboard/apps/catalogue#industry-apps" className="mt-2 inline-flex text-xs font-semibold text-blue-400 hover:text-blue-300">Choose sub-industry →</Link>
                  </div>
                ) : null}
              </div>
            ))}
          ...
          <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-900/30 p-6">
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-400">Your onboarding profile can preselect the most relevant industry and sub-industry App. You can also add one manually at any time.</p>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/35 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Explore the full catalogue only when you want to extend this business into another industry or add another sub-industry App.</p>
          </div>
