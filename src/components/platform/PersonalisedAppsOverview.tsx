...
export function PersonalisedAppsOverview({
  industrySelectionIds,
  organisationName,
}: PersonalisedAppsOverviewProps) {
  ...
  const industryPlatforms = INDUSTRY_PLATFORM_CATALOG.flatMap((platform) => {
    const explicitSpecialisations = platform.specialisations.filter(
      (spec) =>
        selectedIds.has(spec.id) ||
        Boolean(spec.appId && selectedIds.has(spec.appId)),
    );

    if (explicitSpecialisations.length > 0) {
      return [{ platform, specialisations: explicitSpecialisations, legacyFallback: false }];
    }

    // Legacy organisations can have the shared Industry app enabled without a
    // persisted Template selection. Keep the Industry visible, but do not guess
    // and expose every sub-industry Template that happens to share that app id.
    const enabledForPlatform = platform.specialisations.some(
      (spec) => spec.appId && enabledIds.includes(spec.appId),
    );

    if (!hasExplicitIndustrySelection && enabledForPlatform) {
      return [{ platform, specialisations: [], legacyFallback: true }];
    }
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-xl font-bold text-white">
              Apps for {organisationName?.trim() || "your business"}
            </h2>
            <p className="mt-1 max-w-2xl text-sm text-slate-400">
              DigitalGate only shows the Industry Apps and Templates relevant to this business.
              Other industries stay out of the way until you choose to add them.
            </p>
          </div>
          <Link
            href="/dashboard/apps/catalogue"
            className="inline-flex items-center rounded-full border border-slate-600 px-4 py-2 text-sm font-medium text-slate-200 transition hover:border-blue-500 hover:text-white"
          >
      ...
      <section>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Only active industries and selected sub-industry Templates are shown here.
          </p>
        </div>

        {industryPlatforms.length ? (
          <div className="space-y-5">
            {industryPlatforms.map(({ platform, specialisations, legacyFallback }) => (
              <div
                ...
                {specialisations.length ? (
                  ...
                  <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/50 p-4">
                    <p className="text-sm text-slate-300">
                      This Industry App is active, but a specific business Template has not been selected yet.
                    </p>
                    <Link
                      href="/dashboard/apps/catalogue#industry-apps"
                      className="mt-2 inline-flex text-xs font-semibold text-blue-400 hover:text-blue-300"
                    >
          ...
          <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-900/30 p-6">
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-400">
              Your onboarding profile can preselect the most relevant industry and Templates. You can also add one manually at any time.
            </p>
            <Link
              href="/dashboard/apps/catalogue#industry-apps"
              className="mt-4 inline-flex rounded-full border border-slate-600 px-4 py-2 text-sm font-medium text-slate-200 hover:border-blue-500 hover:text-white"
            >
              Choose an industry
            </Link>
          </div>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/35 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Explore the full catalogue only when you want to extend this business into another industry or add specialist capabilities.
            </p>
          </div>
          <Link
            href="/dashboard/apps/catalogue"
