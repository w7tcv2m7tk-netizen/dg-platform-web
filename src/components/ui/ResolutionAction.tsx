...
const modeCopy: Record<ResolutionActionMode, { label: string; hint: string }> = {
  automatic: {
    label: "Fix now",
    hint: "DigitalGate can safely fix this for you.",
  },
  guided: {
    label: "Fix now",
    hint: "DigitalGate will take you to the right place and guide you through the fix.",
  },
  manual: {
    label: "Fix this",
    hint: "Open the exact area where this needs to be corrected.",
  },
};
...
export function ResolutionButton({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center rounded-full bg-sky-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/70 disabled:cursor-not-allowed disabled:opacity-60 ${className}`}
    >
