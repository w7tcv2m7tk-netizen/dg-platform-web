...
export function AppShellLayout({ children, activeOrganisationId, activeOrganisationName, organisations = [], brandTheme = DEFAULT_ORG_BRAND_THEME, chatUserName, showFloatingChat = true, isDemo = false, billingBanner }: { children: ReactNode; activeOrganisationId?: string; activeOrganisationName?: string; organisations?: UserOrganisationSummary[]; brandTheme?: OrgBrandTheme; chatUserName?: string; showFloatingChat?: boolean; isDemo?: boolean; billingBanner?: BillingBannerModel | null; }) {
        ...
        <div className="dg-branded-shell fixed inset-0 z-0 flex overflow-hidden print:static print:inset-auto print:h-auto print:min-h-0 print:overflow-visible" style={orgBrandStyle(brandTheme)}>
          <ChatWidgetProvider userName={chatUserName} showFloatingChat={showFloatingChat}>
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
              ...
              {!isDesktop ? <div className={`fixed inset-0 z-50 ${open ? "" : "pointer-events-none"}`} aria-hidden={!open}>
