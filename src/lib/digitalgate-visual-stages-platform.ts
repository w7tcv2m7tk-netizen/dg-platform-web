...
/** Local stage contract — kept here so Platform does not depend on the
 * superseded Insights stages module that main no longer ships. */
export type PlatformStageDef = {
  name: string;
  anchors: string[];
  html: string;
};

/** Idempotency marker written onto every Platform Overview figure. */
export const PLATFORM_STAGE_OF_ATTR = "data-dg-stage-of";

const PLATFORM_STAGE_KIND = "platform-overview" as const;
const STAGE_OF_ATTR = PLATFORM_STAGE_OF_ATTR;
type StageDef = PlatformStageDef;
