...
export async function PATCH(req: Request) {
  ...
  if (Array.isArray(body.goals) && body.goals.length > 0) {
    const existing = await getOrganisationGoals(session.organisationId); const existingTitles = new Set(existing.map(g => g.title.toLowerCase()));
    for (const g of body.goals.slice(0, 8)) {
      ...
      await createOrganisationGoal(session.organisationId, { title: trimmed, metric: hint.metric, target: hint.target, horizon: "quarter", status: "active" }).catch(() => null); existingTitles.add(trimmed.toLowerCase());
    }
  }
  const [offer, existingProgress] = await Promise.all([effectiveCommercialOffer(session.organisationId), getGen2OnboardingProgress(session.organisationId)]);
  ...
  if (requestedVipSetup?.completedAt) {
    const profile = await getOrganisationBusinessProfile(session.organisationId); const industryTemplates = clientProgress.industryTemplates ?? existingProgress.industryTemplates ?? []; const missing: string[] = [];
...
export async function POST(req: Request) {
  ...
  const [progress, offer, billing] = await Promise.all([
    getGen2OnboardingProgress(session.organisationId), effectiveCommercialOffer(session.organisationId), getOrganisationBillingStatus(session.organisationId),
  ]);
  const body = await req.json().catch(() => ({}));
  ...
  try {
    const checkout = offer ? await createNegotiatedCommercialCheckoutSession({ organisationId: session.organisationId, email: session.email, businessName: session.organisationName, offer, successPath: "/onboarding?checkout=success", cancelPath: "/onboarding?checkout=cancelled" })
