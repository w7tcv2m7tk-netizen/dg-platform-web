import { buildMetaAuthorizeUrl, metaCredentialsConfigured } from "@dg/platform-core";
...
export async function GET(req: Request) {
  ...
  if (!metaCredentialsConfigured()) return NextResponse.json({ error: { code: "meta_not_configured", message: "META_APP_ID / META_APP_SECRET missing on this deployment" } }, { status: 503 });
  ...
  let state: string;
  try { state = createMetaOAuthState(session.organisationId); }
  catch (err) { return NextResponse.json({ error: { code: "meta_state", message: err instanceof Error ? err.message : "Could not create OAuth state" } }, { status: 503 }); }

  const authUrl = buildMetaAuthorizeUrl({ state });
  if (!authUrl.ok) return NextResponse.json({ error: { code: "meta_config", message: authUrl.message } }, { status: 503 });
