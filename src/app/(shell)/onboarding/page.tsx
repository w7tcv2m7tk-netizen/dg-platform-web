...
export default async function OnboardingPage({
  ...
  if (!session) {
    ...
    return (
      <main className="dg-page-main mx-auto max-w-lg px-6 py-16">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-400">Sign in to begin your private Business Operating Platform setup with Aida.</p>
        <a href={`/login?redirect_url=${encodeURIComponent(onboardingPath)}`} className="mt-6 inline-flex min-h-11 items-center rounded-full bg-violet-600 px-5 py-2.5 text-sm font-medium text-white">Sign in to continue</a>
      </main>
    );
  }
    ...
    return (
      <VipOnboardingExperience businessName={session.organisationName}>
        <div className="mx-auto max-w-2xl rounded-3xl border border-violet-300/15 bg-white/[0.035] p-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-300">Your checkout returned successfully. DigitalGate is waiting for Stripe’s verified billing update before continuing.</p>
          <a href="/onboarding?checkout=success" className="mt-5 inline-flex min-h-11 items-center rounded-full bg-violet-600 px-5 py-2.5 text-sm font-medium text-white">Check confirmation</a>
        </div>
  ...
  if (initialProgress.completedAt && !reviewMode) {
    return (
      <VipOnboardingExperience
        ...
        aidaWelcome={`Your DigitalGate workspace for ${session.organisationName} is already configured.`}
        setupFocus={vipPreset?.setupFocus}
      >
        <section className="mx-auto max-w-2xl rounded-3xl border border-violet-300/15 bg-white/[0.035] p-8 text-center shadow-2xl shadow-black/20">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-300">Setup complete</p>
          <h2 className="mt-3 text-2xl font-semibold text-white">Your operating profile is saved</h2>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-white/55">Reviewing setup updates configuration without resetting CRM records, bookings, contacts, documents or transactions.</p>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <a href="/onboarding?review=1" className="inline-flex min-h-11 items-center rounded-full bg-violet-600 px-5 text-sm font-semibold text-white">Review setup with Aida</a>
            <a href="/dashboard" className="inline-flex min-h-11 items-center rounded-full border border-white/10 px-5 text-sm font-semibold text-white/75">Return to platform</a>
          </div>
        </section>
      </VipOnboardingExperience>
    );
  ...
  return (
    <VipOnboardingExperience businessName={session.organisationName} aidaWelcome={vipPreset?.aidaWelcome} setupFocus={vipPreset?.setupFocus}>
