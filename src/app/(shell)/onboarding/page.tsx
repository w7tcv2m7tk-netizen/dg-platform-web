...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ invite?: string; journey?: string; checkout?: string; review?: string; operatorOrg?: string }> }) {
  ...
  if (!session) {
    ...
    return <main className="dg-page-main mx-auto max-w-lg px-6 py-16"><h1 className="text-2xl font-bold text-white">Welcome to DigitalGate</h1><p className="mt-3 text-sm leading-6 text-slate-400">Create your account to start your 14-day free trial and set up your Business Operating Platform with Aida. Already have an account? Sign in to continue.</p><div className="mt-6 flex flex-wrap gap-3"><a href={`/signup/account?redirect_url=${encodeURIComponent(onboardingPath)}`} className="inline-flex min-h-11 items-center rounded-full bg-violet-600 px-5 py-2.5 text-sm font-medium text-white">Create account →</a><a href={`/login?redirect_url=${encodeURIComponent(onboardingPath)}`} className="inline-flex min-h-11 items-center rounded-full border border-white/10 px-5 py-2.5 text-sm font-medium text-white/75">Sign in →</a></div></main>;
  }
