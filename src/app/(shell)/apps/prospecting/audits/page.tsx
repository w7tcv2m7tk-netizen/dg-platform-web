...
export default async function ProspectingAuditsPage() {
  ...
  const [audits, needsAudit] = db && session?.organisationId
    ? await Promise.all([
        listGrowthProspectAudits(organisationGrowthScope(session.organisationId)),
        ...
        {!db ? (
          ...
          <>
            <section>
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">Prospects without a stored audit yet.</p>
                </div>
              ...
              {needsAudit.length === 0 ? (
                <p className="mt-4 text-sm text-slate-500">All current prospects have an audit, or the pipeline is empty.</p>
              ) : (
                <ul className="mt-4 space-y-2">
                  {needsAudit.map((prospect) => (
                    <li key={prospect.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-700/80 bg-slate-950/40 px-4 py-3">
                      <div>
                        <p className="font-medium text-white">{prospect.businessName}</p>
                        <p className="mt-1 text-xs text-slate-500">
                          {[prospect.industry, prospect.location, prospect.websiteUrl].filter(Boolean).join(" · ") || "No website or location yet"}
                        </p>
                      </div>
            ...
            <section>
              ...
              <p className="mt-1 text-sm text-slate-400">Newest first — generate an opportunity report when ready.</p>
              {audits.length === 0 ? (
                <p className="mt-4 text-sm text-slate-500">No audits yet. Run one above.</p>
              ) : (
                <div className="mt-4 space-y-3">
                  {audits.map((audit) => (
                    <article key={audit.id} className="rounded-xl border border-slate-700/80 bg-slate-950/40 px-5 py-4">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <p className="font-medium text-white">{audit.prospect.businessName}</p>
                          <p className="mt-1 text-xs text-slate-500">
                            {GROWTH_ENGINE_STAGE_LABELS[audit.prospect.stage] ?? audit.prospect.stage} · {new Date(audit.auditedAt).toLocaleString("en-AU")} · {findingCount(audit.findings)} findings · {audit.auditVersion}
                          </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                          <RunProspectAuditButton prospectId={audit.prospectId} label="Re-run" />
                          <GenerateProspectReportButton prospectId={audit.prospectId} auditId={audit.id} markSent />
                          <GenerateProspectReportButton prospectId={audit.prospectId} auditId={audit.id} label="Draft only" />
                        </div>
                      </div>
