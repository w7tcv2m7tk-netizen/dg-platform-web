...
export default async function CallCentrePage({
  ...
  const result = session
    ? await listCommunicationSessions({
      ...
      <main className="dg-page-main space-y-6">
        {!session ? (
          <div className="dg-card">
            <p className="text-sm text-slate-400">Sign in to view the call centre.</p>
          </div>
        ) : (
          <>
                  ...
                  {!result.items.length ? (
                    <tr>
                      <td colSpan={6} className="py-6 text-slate-500">
                        No calls yet. Publish an agent and complete a conversation to populate this
                        list.
                      </td>
                    </tr>
                  ) : (
