...
export default async function PmOwnersPage() {
  ...
  const { items } = await listPmPartyContacts(session.organisationId, "owner");

  return (
    <main className="dg-page-main space-y-4">
      <p className="text-sm text-slate-400">
        {session.organisationName} · CRM Contacts linked as owners on Property Management leases
      </p>
      {items.length === 0 ? (
        <div className="dg-card border-dashed border-slate-700">
          <p className="text-slate-400">
            No owners yet. Link an owner Contact from a Property Management lease.
          </p>
        </div>
      ) : (
