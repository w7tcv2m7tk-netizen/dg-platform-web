...
import { fetchOrgGoogleAdsEvidence, fetchOrgMetaAdsEvidence } from "@dg/platform-core";
...
export default async function AdvertisingPage() {
  ...
  const [meta, googleAds] = ctx.session
    ? await Promise.all([fetchOrgMetaAdsEvidence(ctx.session.organisationId), fetchOrgGoogleAdsEvidence(ctx.session.organisationId)])
    : [{ ok: false as const, message: "No active organisation session" }, { ok: false as const, message: "No active organisation session" }];
  return <>
