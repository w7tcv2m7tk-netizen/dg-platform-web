...
export default async function AdvertisingPage() {
  ...
  const [meta, googleAds, microsoftAds, tiktokAds] = ctx.session
  ...
  return <>
    ...
    <main className="dg-page-main space-y-6">
      ...
      <section className="dg-card">{meta.ok&&meta.data.length?<div className="mb-5 grid gap-3 md:grid-cols-4">{[["Spend",`${meta.data.reduce((n,x)=>n+x.performance.spend,0).toFixed(2)}`],["Impressions",meta.data.reduce((n,x)=>n+x.performance.impressions,0).toLocaleString()],["Reach",meta.data.reduce((n,x)=>n+x.performance.reach,0).toLocaleString()],["Clicks",meta.data.reduce((n,x)=>n+x.performance.clicks,0).toLocaleString()]].map(([k,v])=><div key={k} className="rounded-lg border border-slate-800 bg-slate-950/40 p-3"><p className="text-xs text-slate-500">{k} · last 30 days</p><p className="mt-1 text-xl font-semibold text-white">{v}</p></div>)}</div>:<p className="mb-4 text-sm text-amber-400">{meta.ok?"No selected Meta Ads evidence is available.":meta.message}</p>}
        ...
        <p className="mt-2 text-sm text-slate-400">Connect Meta and assign only the ad accounts that belong to this organisation. Authorised performance evidence is combined with Google Ads, Microsoft Advertising and TikTok Ads in the platform intelligence layer.</p>
      ...
      <section className="dg-card">
        <p className="text-xs font-medium uppercase tracking-wide text-sky-400">TikTok Ads evidence</p>
        <h2 className="mt-1 text-lg font-semibold text-white">Last 30 days</h2>
        {tiktokAds.ok&&tiktokAds.data.length?<><div className="mt-4 grid gap-3 md:grid-cols-5">{[["Spend",tiktokAds.data.reduce((n,x)=>n+x.performance.spend,0).toFixed(2)],["Impressions",tiktokAds.data.reduce((n,x)=>n+x.performance.impressions,0).toLocaleString()],["Clicks",tiktokAds.data.reduce((n,x)=>n+x.performance.clicks,0).toLocaleString()],["Conversions",tiktokAds.data.reduce((n,x)=>n+x.performance.conversions,0).toFixed(1)],["Conversion value",tiktokAds.data.reduce((n,x)=>n+x.performance.conversionValue,0).toFixed(2)]].map(([k,v])=><div key={k} className="rounded-lg border border-slate-800 bg-slate-950/40 p-3"><p className="text-xs text-slate-500">{k}</p><p className="mt-1 text-xl font-semibold text-white">{v}</p></div>)}</div>{tiktokAds.data.map(x=><div key={x.advertiser.advertiserId} className="mt-4"><p className="text-sm font-medium text-white">{x.advertiser.name||`TikTok advertiser ${x.advertiser.advertiserId}`}</p><p className="mt-1 text-xs text-slate-500">{x.campaigns.length} campaigns returned · {x.period}</p>{x.campaigns.slice(0,10).map(c=><div key={c.id} className="mt-2 flex items-center justify-between gap-3 rounded-lg border border-slate-800 px-3 py-2 text-sm"><span className="text-slate-300">{c.name}</span><span className="text-xs text-slate-500">{c.clicks.toLocaleString()} clicks · {c.spend.toFixed(2)} spend</span></div>)}</div>)}</>:<p className="mt-3 text-sm text-amber-400">{tiktokAds.ok?"Select a TikTok advertiser above to load live performance evidence.":tiktokAds.message}</p>}
      </section>
