...
export default async function ProductSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "roadmap") return <><header className="dg-page-header"><OperatorCategoryHeader eyebrow="Product" title="Roadmap" question="The entire DigitalGate build — shipped, in progress and future capability across Core, Growth, Intelligence and every Industry App." backHref="/command/product/overview" backLabel="Product" /></header><main className="dg-page-main"><PlatformRoadmapV2Panel /></main></>;
