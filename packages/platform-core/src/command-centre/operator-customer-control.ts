...
export async function getOperatorCustomerControlSnapshot(
  ...
  if (progress.subscriptionActivatedAt && !billing?.commercialStatus && billing?.expectsPlatformBilling) {
    alerts.push("Onboarding records an activated subscription, but no authoritative PlatformSubscription is present.");
  }
  if (billing && ["payment_failed", "past_due", "restricted", "suspended", "cancelled"].includes(billing.kind)) {
