...
export const GEN2_ONBOARDING_STEP_LABELS: Record<Gen2OnboardingStep, string> = {
  welcome: "Welcome", business_identity: "Business identity", business_profile: "Business Profile", goals: "Goals & priorities", plan: "Platform plan", apps: "Apps", billing_cadence: "Monthly or annual", order_summary: "Order summary", stripe: "Activate subscription", connect: "Connect your business", checklist: "Getting started", implementation: "Implementation",
};
...
export type Gen2OnboardingProgress = {
  ...
  /** Underlying Industry App entitlements, e.g. services or finance. */
  industryApps?: string[];
  /** Exact customer-facing sub-industry/template ids, e.g. electrical or cleaning. */
...
export function gen2ChecklistStats(progress: Gen2OnboardingProgress | null) {
  ...
  for (const item of GEN2_CHECKLIST_ITEMS) {
    if ("step" in item && item.step && completed.has(item.step)) { done += 1; continue; }
...
export function emptyGen2Progress(founding = false): Gen2OnboardingProgress {
  ...
  return { version: 1, currentStep: "welcome", completedSteps: [], startedAt: now, updatedAt: now, completedAt: null, platformTier: "professional", industryApps: [], industryTemplates: [], premiumApps: [], billingCadence: "monthly", founding, checklist: {} };
}
