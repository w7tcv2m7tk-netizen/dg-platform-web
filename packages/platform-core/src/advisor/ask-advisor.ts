...
import { llmChat, llmConfigured, llmConfiguredTransports, type LlmProvider } from "../ai/llm";
...
export async function askBusinessAdvisor(
  ...
  if (!llmConfigured()) {
    ...
    await recordAiLedgerEvent({
      ...
      result: { source: fallback.source, question, transports },
    });
    return fallback;
  }
  ...
  try {
    ...
    const answer = result.text.trim() || briefingFallback(input, "llm_error", "empty model response").answer;
    ...
    const fallback = briefingFallback(input, "llm_error", message);
    await recordAiLedgerEvent({
      organisationId: input.organisationId,
      actorId: input.actorId,
