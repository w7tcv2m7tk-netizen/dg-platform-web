...
Gets the central calendar working immediately while partner programmes progress. **iCal still has a race window** — cron reduces doubles but native APIs (Stage 2) are required for near-real-time.
...
- [CONNECTOR-ENGINE.md](./CONNECTOR-ENGINE.md) — shared Connector Framework (Acc OTA adapters plug in here)
- [PROPERTY-SYNDICATION.md](./PROPERTY-SYNDICATION.md) — RE Listing Hub (Domain / REA) — same adapter idea  
