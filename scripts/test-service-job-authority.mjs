...
describe("canonical services.jobs mapping", () => {
  ...
  it("gives Members organisation-scope industry view but only assigned commerce", () => {
    const source = readFileSync(
      path.join(__dirname, "../packages/platform-core/src/access/defaults.ts"),
      "utf8",
    );
    assert.match(source, /\["crm", "industry", "growth", "intelligence", "documents", "communications"\]/);
    assert.match(source, /\["commerce"\], \["view", "create"\], "assigned"/);
  });
});
...
describe("Service Job pages", () => {
  it("enforce services.jobs.read before loading job data", () => {
    const detail = readFileSync(
      path.join(__dirname, "../src/app/(shell)/apps/services/jobs/[id]/page.tsx"),
      "utf8",
    );
    const list = readFileSync(
      path.join(__dirname, "../src/app/(shell)/apps/services/jobs/page.tsx"),
      "utf8",
    );
